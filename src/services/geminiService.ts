...
const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY || firebaseConfig.apiKey || "" });
...
export const setCache = (key: string, data: any, ttlMs: number = 3600000) => {
...
export const flushAILogs = async () => {
  ...
  // Condense logs by summarizing usage per feature
  const summary = batchToUpload.reduce((acc, curr) => {
    if (!acc[curr.feature]) acc[curr.feature] = { hits: 0, estimatedTokens: 0, errors: 0 };
    acc[curr.feature].hits += 1;
    acc[curr.feature].estimatedTokens += curr.tokensEstimated;
    if (!curr.success) acc[curr.feature].errors += 1;
    return acc;
  }, {} as Record<string, { hits: number, estimatedTokens: number, errors: number }>);
  ...
  try {
    ...
    console.log("Telemetry: Flushed batched AI logs summary to Firestore: ", summary);
  } catch (error) {
...
export const analyzeSymptoms = async (symptoms: string, petInfo: string, imageBase64?: string): Promise<SymptomAnalysis | null> => {
  const cacheKey = `${normalizeQuery(symptoms)}-${petInfo}-${imageBase64?.slice(-50)}`;
  const cached = getCache(cacheKey);
  if (cached) return cached;

  // PRIMARY LOCAL SHIELD: Text only bypass
  if (!imageBase64) {
  ...
  try {
    const analysisPrompt = imageBase64 
      ? `Perform a high-fidelity multimodal clinical assessment of this pet's visual symptoms.
         Context: ${optimizeContext(petInfo)}. 
         Observation Intent: ${optimizeContext(symptoms)}.
      ...
      : `Analyze these symptoms for a pet. Context: ${optimizeContext(petInfo)}. Symptoms: ${optimizeContext(symptoms)}. Focus on triage and ruling out immediate emergencies. Use Google Search to check for any current pet health alerts or common outbreaks in the region if relevant.`;
    ...
    logAIInteraction({ feature: 'analyzeSymptoms', tokensEstimated: imageBase64 ? 500 : 100, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
    
    return result;
  } catch (error: any) {
    logAIInteraction({ feature: 'analyzeSymptoms', tokensEstimated: 0, timestamp: new Date().toISOString(), success: false, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
...
export const generateSOAPNote = async (transcript: string): Promise<SOAPNote | null> => {
  ...
  try {
    const response = await ai.models.generateContent({
      model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash", // Use lite for simple record drafting
    ...
    logAIInteraction({ feature: 'generateSOAPNote', tokensEstimated: 500, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
    return result;
  } catch (error: any) {
    logAIInteraction({ feature: 'generateSOAPNote', tokensEstimated: 0, timestamp: new Date().toISOString(), success: false, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
 ...
 */
export const getChatSession = (context: string, persona: AIPersona = 'doctor', summary?: string) => {
  const limitCheck = checkLimits('chat-session', AICost.GROUNDING_SEARCH, 1000);
  if (!limitCheck.allowed && limitCheck.reason === 'DAILY_CAP') {
    throw new Error(LIMIT_REACHED_MSG);
  }

  logAIInteraction({ feature: 'getChatSession', tokensEstimated: 500, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
...
export const getPetAdvice = async (query: string, context: string): Promise<string> => {
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      model: isSearchNeeded ? import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" : import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash",
      contents: query,
      config: {
        maxOutputTokens: 200,
...
- IMPORTANT: Keep in mind the context is India. Be aware of local environmental factors (e.g., intense heat, monsoon seasons, and how they impact pet health/common diseases like Tick Fever or Parvovirus).`,
    ...
    try {
      logAIInteraction({ feature: 'getPetAdvice', tokensEstimated: 200, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
    } catch (e) {
...
export const parseMedicalRecord = async (imageBase64: string): Promise<Partial<HealthRecord> | null> => {
  ...
  try {
    ...
    logAIInteraction({ feature: 'parseMedicalRecord', tokensEstimated: 400, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
    return result;
  } catch (error: any) {
    logAIInteraction({ feature: 'parseMedicalRecord', tokensEstimated: 0, timestamp: new Date().toISOString(), success: false, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
...
export const getDifferentialDiagnosis = async (pet: Pet, history: HealthRecord[], clinicalQuery: string): Promise<ClinicalAnalysis> => {
  ...
  try {
    ...
    logAIInteraction({ feature: 'getDifferentialDiagnosis', tokensEstimated: 800, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
    return result;
  } catch (error: any) {
    logAIInteraction({ feature: 'getDifferentialDiagnosis', tokensEstimated: 0, timestamp: new Date().toISOString(), success: false, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
...
// from getClinicalAssessment
  ...
  try {
    ...
    logAIInteraction({ feature: 'getClinicalAssessment', tokensEstimated: 500, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
    return result;
  } catch (error: any) {
    logAIInteraction({ feature: 'getClinicalAssessment', tokensEstimated: 0, timestamp: new Date().toISOString(), success: false, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
...
export const summarizeConversation = async (messages: { role: 'user' | 'ai'; text: string }[]): Promise<string> => {
  try {
    ...
    logAIInteraction({ feature: 'summarizeConversation', tokensEstimated: 300, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
 ...
 * Logic: Uses gemini-2.5-flash for fast and accurate speech-to-text.
 */
...
export const getPatientSnapshot = async (pet: Pet, healthHistory: HealthRecord[], appointment: Appointment): Promise<PatientSnapshot | null> => {
  ...
  try {
    ...
    logAIInteraction({ feature: 'getPatientSnapshot', tokensEstimated: 600, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
    return result;
  } catch (error: any) {
    if (error?.status === 429 || error?.message?.includes("RESOURCE_EXHAUSTED")) {
...
export const getProactiveSuggestions = async (pet: Pet, healthHistory: HealthRecord[]): Promise<ProactiveSuggestion[]> => {
  ...
  try {
    ...
    logAIInteraction({ feature: 'getProactiveSuggestions', tokensEstimated: 400, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
...
export const summarizeMedicalRecord = async (record: HealthRecord): Promise<string> => {
  ...
  try {
    ...
    logAIInteraction({ feature: 'summarizeMedicalRecord', tokensEstimated: 200, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
...
export const getZoolInsights = async (pets: any[], records: HealthRecord[]): Promise<ZoolInsight[]> => {
  ...
  try {
    ...
    try {
      logAIInteraction({ feature: 'getZoolInsights', tokensEstimated: 800, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
    } catch (e) {
...
export const getRegionalHealthAlerts = async (locationInput: string | { lat: number, lng: number }): Promise<PublicAlert[]> => {
  ...
  try {
    ...
    logAIInteraction({ feature: 'getRegionalHealthAlerts', tokensEstimated: 400, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
    return result;
  } catch (error: any) {
    if (error?.status === 429 || error?.message?.includes("RESOURCE_EXHAUSTED")) {
...
export const getBreedInsight = async (breed: string, type: string): Promise<BreedInsight | null> => {
  ...
  try {
    ...
    setCache(cacheKey, result);
    logAIInteraction({ feature: 'getBreedInsight', tokensEstimated: 600, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
    return result;
  } catch (error) {
 ...
 * Logic: Cross-references pet age, breed, and history with medical standards to suggest next reminders.
 */
export const getAIReminderSuggestions = async (pets: any[], records: HealthRecord[]): Promise<SuggestedReminder[]> => {
  ...
  try {
    const response = await ai.models.generateContent({
      model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash",
      contents: `Based on the following pet data, suggest 2-3 specific upcoming health reminders (vaccinations, checkups, or deworming) for each pet. 
      Consider breed-specific predispositions and age-appropriate care.
      ...
      config: {
        ...
        maxOutputTokens: 1000,
        responseMimeType: "application/json",
        systemInstruction: "You are the ZooL Health Intelligence assistant. Provide precise veterinary reminder suggestions. IMPORTANT: Respond ONLY with a valid JSON array of objects: Array<{ petId: string, petName: string, type: 'vaccination'|'checkup'|'medication'|'other', title: string, suggestedDate: string (YYYY-MM-DD), reason: string, priority: 'low'|'medium'|'high' }>. No extra text."
      }
    });

    const result = safeJSONParse(response.text, []) as SuggestedReminder[];
    if (result.length > 0) {
      setCache(cacheKey, result);
    }
    try {
      logAIInteraction({ feature: 'getAIReminderSuggestions', tokensEstimated: 600, timestamp: new Date().toISOString(), success: true, model: import.meta.env.VITE_GEMINI_MODEL || "gemini-2.5-flash" });
    } catch (e) {
      console.warn("Telemetry log failed, but reminders retrieved.", e);
    }
    return result;
  } catch (error: any) {
    if (error?.status === 429 || error?.message?.includes("RESOURCE_EXHAUSTED")) {
      console.warn("AI Quota exhausted (Reminders). Optimization logic engaged.");
      return [];
    }
    console.error("AI Reminder suggestion failed:", error);
    return [];
  }
};
