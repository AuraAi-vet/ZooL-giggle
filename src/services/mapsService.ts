...
export const findNearbyPlaces = async (location: Location, type: string): Promise<Place[]> => {
  ...
  // Attempt to recover from localForage cache offline first if network is unavailable
  if (typeof navigator !== 'undefined' && !navigator.onLine) {
    try {
      const offlineCached = await mapsCacheStore.getItem<Place[]>(cacheKey);
      if (offlineCached && offlineCached.length > 0) {
        console.log("[Offline Maps] Retrieved cached OSM places from localForage:", offlineCached);
  ...
  try {
    ...
    if (places.length > 0) {
      ...
      mapsCacheStore.setItem(cacheKey, places).catch(err => {
    ...
    try {
      const offlineCached = await mapsCacheStore.getItem<Place[]>(cacheKey);
      if (offlineCached && offlineCached.length > 0) {
        console.log("[Offline Maps Backed-up Fallback] OSM down, loaded from localForageCache:", offlineCached);
        return offlineCached;
      }
    } catch (err) {
