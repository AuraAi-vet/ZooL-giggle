...
const mapsCacheStore = localforage.createInstance({
  name: 'zool-maps',
  storeName: 'nearby-places'
});
...
export const findNearbyPlaces = async (location: Location, type: string): Promise<Place[]> => {
  ...
  // AGGRESSIVE CACHING: Try memory cache first
  if (cached && (now - cached.timestamp < CACHE_TTL)) {
    return cached.data;
  }

  // AGGRESSIVE CACHING: Check IndexedDB (localForage) BEFORE making network request regardless of offline status
  try {
    const offlineCached = await mapsCacheStore.getItem<{ timestamp: number, data: Place[] }>(cacheKey);
    if (offlineCached && (now - offlineCached.timestamp < CACHE_TTL)) {
      console.log("[Aggressive Cache] Retrieved cached OSM places from IndexedDB:", offlineCached.data);
      // populate memory cache for faster subsequent reads
      placesApiCache.set(cacheKey, offlineCached);
      return offlineCached.data;
    }
  } catch (err) {
    console.warn("localForage offline read failed:", err);
  }
  ...
  try {
    ...
    if (places.length > 0) {
      const cacheObject = { timestamp: Date.now(), data: places };
      placesApiCache.set(cacheKey, cacheObject);
      // Persistent localForage write for complete offline functionality
      mapsCacheStore.setItem(cacheKey, cacheObject).catch(err => {
        console.warn("localForage cache write failed:", err);
      });
      return places;
    } else {
      return [];
    ...
    console.warn("OSM API error, trying localForage backup fallback:", error);
    try {
      const offlineCached = await mapsCacheStore.getItem<{ timestamp: number, data: Place[] }>(cacheKey);
      if (offlineCached && offlineCached.data.length > 0) {
        console.log("[Offline Maps Backed-up Fallback] OSM down, loaded from localForageCache:", offlineCached.data);
        return offlineCached.data;
      }
    } catch (err) {
      console.warn("localForage backup fallback failed:", err);
    }
    return [];
  }
};
