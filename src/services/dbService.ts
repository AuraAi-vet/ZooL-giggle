...
import { UserProfile, PetProfile, AppNotification } from '../types';
...
export const subscribeToOwnerAppointments = (ownerUid: string, callback: (appointments: Appointment[]) => void) => {
  let initial = true;
  let fallbackApts: Appointment[] = [];
  try {
...
export const subscribeToNotifications = (userId: string, callback: (notifications: AppNotification[]) => void) => {
  ...
  const unsubscribe = onSnapshot(q, (snap) => {
    ...
    if (StringErr.includes('Missing or insufficient permissions')) {
      const pushNotifs = () => {
        try {
          const notifs = JSON.parse(localStorage.getItem('mock_notifs') || '[]').filter((n: AppNotification) => n.userId === userId);
          callback(notifs.sort((a: AppNotification, b: AppNotification) => b.timestamp - a.timestamp));
        } catch (e) {}
      };
