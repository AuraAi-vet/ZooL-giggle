...
import { TelehealthZoomCall } from '../components/TelehealthZoomCall';
...
export function AppointmentsView({ 
  ...
  const [activeZoomCall, setActiveZoomCall] = useState<Appointment | null>(null);
  ...
  return (
    ...
    >
            ...
            {upcoming.length > 0 ? upcoming.map((app, idx) => (
              ...
              >
                ...
                <div className="flex gap-4">
                  {role === 'vet' ? (
                    <>
                      {app.type === 'telehealth' && app.status === 'scheduled' && (
                        <>
                          <motion.button 
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                            onClick={() => setActiveZoomCall({ ...app, testMode: true } as any)}
                            className="flex-1 py-4.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 transition-all"
                          >
                            <Activity size={18} />
                            <span>Conn. Test</span>
                          </motion.button>
                          <motion.button 
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                            onClick={() => setActiveZoomCall(app)}
                            className="flex-[2] py-4.5 bg-blue-600 hover:bg-blue-500 text-white rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 shadow-xl shadow-blue-600/20 transition-all"
                          >
                            <Video size={18} />
                    ...
                    <>
                      {app.type === 'telehealth' && app.status === 'scheduled' && (
                        <motion.button 
                          ...
                          whileTap={{ scale: 0.98 }}
                          onClick={() => setActiveZoomCall({ ...app, testMode: true } as any)}
                          className="px-6 py-4.5 bg-slate-100/50 hover:bg-slate-100 border border-slate-200 text-slate-700 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all font-sans"
                        >
                          Test
                        </motion.button>
                      )}
                      <motion.button 
                        ...
                        whileTap={{ scale: 0.98 }}
                        onClick={() => {
                          if (app.type === 'telehealth') {
                            setActiveZoomCall(app);
                          } else {
                            toast.info("No Zoom configuration required for Physical Presence clinics. Please follow mapping directions.");
                          }
                        }}
      ...
      {/* HIPAA Compliant Telehealth Zoom SDK Modal overlay */}
      {activeZoomCall && (
        <TelehealthZoomCall
          isOpen={!!activeZoomCall}
          onClose={() => setActiveZoomCall(null)}
          appointmentId={activeZoomCall.id}
          vetName={activeZoomCall.vetName}
          petName={pets.find(p => p.id === activeZoomCall.petId)?.name || 'Companion'}
          testMode={(activeZoomCall as any).testMode}
        />
      )}
    </motion.div>
  );
