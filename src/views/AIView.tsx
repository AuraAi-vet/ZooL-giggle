...
export function AIView({ 
  ...
  const [showMascotInfo, setShowMascotInfo] = React.useState(false);
            ...
            <div className="space-y-1 relative">
              ...
              <div className="flex items-center flex-wrap gap-2">
                ...
                <button 
                  ...
                  className="text-[10px] font-bold uppercase text-amber-700 bg-amber-50 hover:bg-amber-100 px-2 py-0.5 rounded-lg flex items-center gap-1 transition-all cursor-pointer shadow-sm active:scale-95 border border-amber-200/30"
                >
                  <Sparkles size={10} className="text-amber-600 animate-spin" style={{ animationDuration: '4s' }} />
                  Mascot Heritage
                </button>
              </div>
              ...
              <AnimatePresence>
                {showMascotInfo && (
                  <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0, scale: 1 }}
                    exit={{ opacity: 0, y: -10, scale: 0.95 }}
                    className="absolute top-14 left-0 w-80 md:w-96 bg-white border border-amber-200/40 p-6 rounded-[2.5rem] shadow-2xl z-50 space-y-4"
                  >
                    <div className="flex items-start justify-between">
                      <div className="space-y-0.5">
                        <h4 className="text-xs font-bold text-amber-900 uppercase tracking-wider flex items-center gap-1.5 font-display">
                          <Sparkles size={12} className="text-amber-500" />
                          RuRu Mascot DNA & Heritage
                        </h4>
                        <p className="text-[10px] font-black uppercase tracking-widest text-amber-500">
                          Emotional Caregiver Synapse
                        </p>
                      </div>
                      <button 
                    ...
                    <div className="space-y-3.5">
                      <div className="flex items-center gap-4 bg-amber-50/60 p-4 rounded-[1.75rem] border border-amber-100/40">
                        <div className="flex-shrink-0 bg-white p-2.5 rounded-2xl shadow-sm border border-slate-100">
                          <RuRuIcon role="guest" size={60} status="listening" />
                        </div>
                        <div className="space-y-1">
                          <p className="text-xs font-black uppercase text-amber-950 tracking-wide">Feline-Owl Clinical Hybrid</p>
                          <p className="text-[10px] text-slate-500 leading-normal">
                            Specifically designed to unify precision intelligence with high-empathy animal traits for comfortable caregiving.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-2 text-[11px]">
                        <div className="p-3 bg-slate-50 border border-slate-100 rounded-2xl space-y-1">
                          <span className="font-extrabold text-slate-900 flex items-center gap-2">
                            ...
                            🦉 Wise Owl Triage Watch
                          </span>
                          <p className="text-[10px] text-slate-500 pl-3.5 leading-normal">
                            Represents 24/7 night watches, systematic logic checkups, and quick hospital routing directories.
                          </p>
                        </div>
                        ...
                        <div className="p-3 bg-slate-50 border border-slate-100 rounded-2xl space-y-1">
                          <span className="font-extrabold text-slate-900 flex items-center gap-2">
                            ...
                            🐱 Playful Domestic Empathy
                          </span>
                          <p className="text-[10px] text-slate-500 pl-3.5 leading-normal">
                            Fosters organic bonding and oxytocin release, keeping pet parents calm in emergency situations.
                          </p>
                        </div>
                        ...
                        <div className="p-3 bg-slate-50 border border-slate-100 rounded-2xl space-y-1">
                          <span className="font-extrabold text-slate-900 flex items-center gap-2">
                            ...
                            🤖 Robotic Visor Scanner
                          </span>
                          <p className="text-[10px] text-slate-500 pl-3.5 leading-normal">
                            Conveys absolute accuracy in calculations, pharmaceutical dosing guides, and fast sync streams.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="pt-2.5 border-t border-slate-100 space-y-2">
                        <div className="grid grid-cols-3 gap-1.5 text-center">
                          <div className="p-2.5 bg-slate-50 border border-slate-100 rounded-xl">
                            <span className="block text-sm font-black text-slate-900">98.4%</span>
                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block">Trust</span>
                          </div>
                          <div className="p-2.5 bg-slate-50 border border-slate-100 rounded-xl">
                            <span className="block text-sm font-black text-emerald-600">94.2%</span>
                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block">Accuracy</span>
                          </div>
                          <div className="p-2.5 bg-slate-50 border border-slate-100 rounded-xl">
                            <span className="block text-sm font-black text-indigo-600">300ms</span>
                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block">SOS Resp</span>
                          </div>
                        </div>
                      </div>
                    </div>
