...
export function ServicesView({ 
  ...
  const { 
    ...
    cachedNearbyPlaces, setCachedNearbyPlaces,
  ...
  const [nearbyPlaces, setNearbyPlaces] = useState<Place[]>(cachedNearbyPlaces);
  ...
  const handleBookingPayment = async () => {
    ...
    // Call the booking service prop to record the intent
    onBook(selectedPetId, bookingService.name, bookingDate, selectedVetId, bookingReason);
    ...
    try {
      ...
      if (session.url) {
        // Save booking intent to local storage to finalize after return
        ...
        {showMap && currentLoc && (
          ...
          >
            <motion.div 
              initial={{ scale: 0.95 }}
              animate={{ scale: 1 }}
              className="w-full h-[500px] md:h-[700px] rounded-[2.5rem] overflow-hidden shadow-2xl border border-ruru-navy/10 bg-white/95 backdrop-blur-3xl mb-12"
            >
              <ServiceMap 
                center={currentLoc} 
            ...
            >
              ...
              <div className="flex gap-4 pt-8">
                <motion.button 
                  ...
                  className="w-full py-8 bg-slate-900 text-white rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.4em] shadow-3xl shadow-slate-900/30 disabled:opacity-50 transition-all flex items-center justify-center gap-4 italic group"
                >
                  {isProcessingPayment ? (
                    <>
                    ...
                    <>
                      Execute Secure Session <ChevronRight size={20} className="group-hover:translate-x-2 transition-transform" />
                    </>
                  )}
                </motion.button>
              </div>
