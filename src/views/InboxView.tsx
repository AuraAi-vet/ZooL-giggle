...
export default function InboxView() {
  ...
  const getHeader = (msg: GmailMessage, headerName: string) => {
    ...
    const header = msg.payload.headers.find((h: GmailPayloadHeader) => h.name === headerName);
    ...
    <div className="p-6 h-full flex flex-col bg-slate-50">
      ...
      {isComposeOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-xl border border-slate-200 w-full max-w-lg flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
              <h3 className="font-bold text-slate-800">New Message</h3>
              <button onClick={() => setIsComposeOpen(false)} className="p-1 text-slate-400 hover:text-slate-600 rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
