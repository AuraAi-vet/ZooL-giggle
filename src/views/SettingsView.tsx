...
import { SettingsViewSkeleton } from '../components/DashboardSkeleton';
...
export default function SettingsView() {
  ...
  const [language, setLanguage] = useState(contextLang);
      ...
      if (auth.currentUser) {
        ...
        if (userProfile) {
          ...
          if (userProfile.language) {
            const val = userProfile.language as 'en' | 'ml' | 'hi';
            setLanguage(val);
            setContextLang(val);
          }
  ...
  const handleLanguageChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const val = e.target.value as 'en' | 'ml' | 'hi';
    setLanguage(val);
    setContextLang(val);
  };
  ...
  return (
    <div className="p-6 md:p-8 min-h-screen bg-[#F8FAFC]">
      <header className="mb-8 flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">{t('settings')}</h1>
        ...
        <div className="md:col-span-4 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-3xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.02)] border border-slate-200/60 p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="font-bold text-lg text-slate-800">{t('selectLanguage')}</h2>
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.02)] border border-slate-200/60 p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="font-bold text-lg text-slate-800">{t('notifications')}</h2>
            </div>
