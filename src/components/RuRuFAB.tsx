...
export const RuRuFAB: React.FC<RuRuFABProps> = ({ onOpenAI, onNavigate, isOpen, setIsOpen, role = 'guest', status }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="absolute inset-y-0 right-0 w-8 bg-ruru-teal/30 blur-xl rounded-l-full group-hover:bg-ruru-magenta/50 transition-all opacity-0 group-hover:opacity-100" />
        
        <div className="relative">
          <RuRuIcon size={64} role={role} status={status || (showOptions ? 'idle' : 'listening')} className="shadow-[0_20px_50px_rgba(52,181,199,0.3)] shadow-ruru-teal/40 group-hover:shadow-[0_20px_50px_rgba(161,80,160,0.4)] transition-shadow duration-700 rounded-full bg-ruru-navy-light pt-1.5" />
