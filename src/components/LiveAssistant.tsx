...
export const LiveAssistant: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  // Initialize and Connect Live API Session
  const connectSession = async () => {
    setIsConnecting(true);
    setError(null);
    setIsConnected(false);

    // Stop and clean up any existing session / audio
    if (sessionRef.current) {
      try {
        sessionRef.current.close();
      } catch (err) {
        console.warn("Error tearing down preceding voice session:", err);
      }
    ...
    try {
      ...
      const session = await ai.live.connect({
        model: import.meta.env.VITE_GEMINI_LIVE_MODEL || "gemini-2.0-flash-exp",
        config: {
        ...
        callbacks: {
          ...
          onerror: (err) => {
            ...
            setError("Connection error. The service may be throttled or offline. Please click Reconnect to retry.");
            setIsConnecting(false);
          },
          onclose: () => {
            setIsConnected(false);
      ...
      setError("Failed to initialize AI session. Click Reconnect to try again.");
      setIsConnecting(false);
    }
  };
      ...
      <div className="flex-1 flex flex-col items-center justify-center space-y-16 w-full max-w-md relative z-10">
        ...
        <div className="text-center space-y-6 min-h-[140px] flex flex-col justify-center w-full">
          <AnimatePresence mode="wait">
            {error ? (
              <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center gap-4 bg-red-400/10 px-6 py-4 rounded-[2.5rem] border border-red-400/20 max-w-sm mx-auto">
                <div className="flex items-center justify-center gap-3 text-red-400">
                  <AlertCircle size={20} className="shrink-0" />
                  <p className="text-sm font-medium tracking-tight text-center">{error}</p>
                </div>
                <button
                  onClick={() => setReconnectCount(c => c + 1)}
                  disabled={isConnecting}
                  className="px-6 py-2.5 bg-white text-ruru-navy hover:bg-ruru-pink-pale hover:text-ruru-magenta active:scale-95 disabled:opacity-50 disabled:scale-100 rounded-full text-xs font-black uppercase tracking-widest shadow-md transition-all duration-300 cursor-pointer"
                >
