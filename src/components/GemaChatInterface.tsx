...
interface GemaChatProps {
  petContext?: PetProfile | undefined;
  onBookAction: () => void;
}
  ...
  return (
    <div className="relative bg-white/80 backdrop-blur-2xl rounded-3xl p-6 border border-white/50 shadow-[0_8px_32px_rgba(0,0,0,0.06)] flex flex-col gap-4 h-[400px] md:h-[500px] overflow-hidden">
      ...
      <h2 className="relative z-10 text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mb-2 flex items-center justify-between gap-2">
        ...
        <button onClick={clearChat} aria-label="Clear Chat" className="p-2 rounded-lg text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-colors tooltip focus:outline-none" title="Clear Chat">
           <Trash2 size={16} />
        </button>
      </h2>
      ...
      <div className="flex gap-2 relative z-10 p-1 bg-slate-50 border border-slate-200 rounded-2xl">
        ...
        />
        <button onClick={sendMessage} aria-label="Send Message" className="aspect-square flex items-center justify-center bg-slate-900 text-white rounded-xl hover:bg-sky-600 hover:shadow-[0_0_15px_rgba(14,165,233,0.4)] transition-all action-bounce m-1 px-4">
          <Send size={16} />
        </button>
      </div>
    </div>
