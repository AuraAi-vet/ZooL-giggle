...
export function ServiceMap({ center, places, onPlaceSelect, onMarkerClick, onBookInit, onClosePopup, isLoading, selectedPlaceId }: ServiceMapProps) {
      ...
      <div className="absolute inset-0 pointer-events-none z-10 flex flex-col justify-between p-4">
        <div className="flex gap-2 pointer-events-auto self-start">
           <div className="bg-white/80 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/20 shadow-lg flex items-center gap-2">
            <span className="w-1.5 h-1.5 bg-ruru-teal rounded-full animate-pulse" />
            <span className="text-[9px] font-black uppercase tracking-widest text-ruru-navy">OSM Powered</span>
          </div>
        </div>
          ...
          {(selectedPlaceId || localSelectedPlace) && (
            ...
            >
              <div className={cn(
                ...
                (selectedPlaceId ? places.find(p => p.id === selectedPlaceId) : localSelectedPlace)?.type === 'vet' ? "bg-ruru-teal" : "bg-amber-500"
              )} />
              <div className="flex justify-between items-start mb-2">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-black uppercase tracking-widest text-ruru-navy/60">
                      {(selectedPlaceId ? places.find(p => p.id === selectedPlaceId) : localSelectedPlace)?.type}
                    </span>
                    {(selectedPlaceId ? places.find(p => p.id === selectedPlaceId) : localSelectedPlace)?.isSubsidized && (
                  ...
                  <h4 className="font-brand text-2xl text-ruru-navy line-clamp-1 pr-8">
                    {(selectedPlaceId ? places.find(p => p.id === selectedPlaceId) : localSelectedPlace)?.name}
                  </h4>
                </div>
                <button 
                  onClick={() => {
              ...
              <div className="space-y-4 mb-8">
                <p className="text-[11px] text-ruru-navy/60 flex items-center gap-2 font-medium">
                  ...
                  <span className="truncate">{(selectedPlaceId ? places.find(p => p.id === selectedPlaceId) : localSelectedPlace)?.address}</span>
                </p>
                <div className="flex items-center gap-4">
                  {(selectedPlaceId ? places.find(p => p.id === selectedPlaceId) : localSelectedPlace)?.rating && (
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-amber-50 text-amber-700 rounded-[1.25rem] border border-amber-100">
                      ...
                      <span className="text-xs font-black">{(selectedPlaceId ? places.find(p => p.id === selectedPlaceId) : localSelectedPlace)?.rating}</span>
                    </div>
                  )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <motion.a
                  ...
                  whileTap={{ scale: 0.98 }}
                  href={getDirectionsUrl((selectedPlaceId ? places.find(p => p.id === selectedPlaceId) : localSelectedPlace)!.location)}
                  target="_blank"
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={() => {
                    const p = (selectedPlaceId ? places.find(p => p.id === selectedPlaceId) : localSelectedPlace) as Place;
                    if (onBookInit) {
                      onBookInit(p);
                    } else {
                      onPlaceSelect?.(p);
