...
import { Sparkles, TrendingUp, IndianRupee } from 'lucide-react';
...
const mockProjectionData = [
  ...mockHistoricalData,
  { month: 'Jun', revenue: 640000, isProjection: true },
  { month: 'Jul', revenue: 710000, isProjection: true },
  { month: 'Aug', revenue: 760000, isProjection: true },
];
...
export default function FinancialProjection() {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-[0_4px_24px_-8px_rgba(0,0,0,0.08)] border border-slate-200/60 p-6 md:p-8 flex flex-col h-full relative overflow-hidden">
      <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-50/50 rounded-bl-full blur-3xl pointer-events-none -mr-10 -mt-10"></div>
      <div className="flex items-center justify-between mb-6 relative z-10">
        <div>
          <h2 className="text-xl font-bold flex items-center gap-2 text-slate-800">
            <TrendingUp className="w-5 h-5 text-indigo-500" />
            Financial Projections (INR)
          </h2>
          <p className="text-sm text-slate-500">Analyze booking history to predict future revenue trends and UPI payments.</p>
        </div>
        {!isProjected ? (
          <button 
        ...
        <div className="w-full md:w-1/3 flex flex-col gap-4">
           {isProjected && (
             <div className="bg-indigo-50/50 border border-indigo-100 rounded-2xl p-5 animate-in fade-in duration-500">
               ...
               <p className="text-sm text-slate-600 mb-4">
                 Based on current UPI velocity and seasonal trends, your expected revenue is projected to grow by <strong className="text-emerald-600">28%</strong> over the next quarter.
               </p>
               <div className="flex items-center justify-between bg-white p-3 rounded-xl border border-indigo-50">
                 ...
                 <span className="font-bold text-indigo-700 flex items-center"><IndianRupee className="w-4 h-4"/>{formatINR(2110000)}</span>
               </div>
             </div>
           )}
           ...
           <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5">
              ...
              <div className="text-2xl font-bold text-slate-900 flex items-center gap-1">
                <IndianRupee className="w-6 h-6 text-slate-400" />{formatINR(1640000)}
              </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <Tooltip 
                ...
                formatter={(value: number, name: string, props: any) => [`₹${formatINR(value)}`, props.payload.isProjection ? 'Projected Revenue' : 'Actual Revenue']}
              />
              <Area 
                type="monotone" 
                dataKey="revenue" 
