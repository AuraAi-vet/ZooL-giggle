      ...
      {showBulkPriceModal && (
        <div className="absolute inset-0 z-30 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl space-y-4 relative">
             <button onClick={() => setShowBulkPriceModal(false)} aria-label="Close modal" className="absolute top-4 right-4 text-slate-400 hover:text-slate-600">
      ...
      {selectedItems.length > 0 && (
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-40 bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center justify-between min-w-[400px] animate-in slide-in-from-bottom-8">
           ...
           <div className="flex items-center gap-2">
             ...
             <button onClick={handleBulkDelete} aria-label="Delete selected" className="p-1.5 ml-2 text-rose-400 hover:text-rose-300 hover:bg-slate-800 rounded-lg transition-colors"><Trash2 className="w-4 h-4"/></button>
           </div>
        </div>
      )}
            ...
            {services.map(item => (
              <div key={item.id} className={`group bg-white border ${selectedItems.includes(item.id) ? 'border-indigo-400 ring-2 ring-indigo-400/20' : item.isActive === false ? 'border-dashed border-slate-200 opacity-60' : 'border-slate-200'} rounded-2xl p-5 hover:border-indigo-300 hover:shadow-md transition-all relative`}>
                 <div className="absolute top-4 left-4 z-10">
                   <button onClick={() => toggleSelection(item.id)} aria-label="Select item" className={`p-0.5 rounded-md ${selectedItems.includes(item.id) ? 'text-indigo-600' : 'text-slate-300 hover:text-indigo-400'}`}>
                 ...
                 <div className="absolute top-4 right-4 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button onClick={() => handleEditStart(item)} aria-label="Edit item" className="p-1.5 text-slate-400 hover:text-indigo-600 bg-slate-50 rounded-md">
                       <Edit2 className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDelete(item.id)} aria-label="Delete item" className="p-1.5 text-slate-400 hover:text-rose-500 bg-slate-50 rounded-md">
          ...
          <div className="space-y-4">
            {products.map(item => (
              <div key={item.id} className={`group bg-white border ${selectedItems.includes(item.id) ? 'border-emerald-400 ring-2 ring-emerald-400/20' : item.isActive === false ? 'border-dashed border-slate-200 opacity-60' : 'border-slate-200'} rounded-2xl p-5 hover:border-emerald-300 hover:shadow-md transition-all relative`}>
                 <div className="absolute top-4 left-4 z-10">
                   <button onClick={() => toggleSelection(item.id)} aria-label="Select item" className={`p-0.5 rounded-md ${selectedItems.includes(item.id) ? 'text-emerald-600' : 'text-slate-300 hover:text-emerald-400'}`}>
                     {selectedItems.includes(item.id) ? <CheckSquare className="w-5 h-5" /> : <Square className="w-5 h-5" />}
                   </button>
                 </div>
                 <div className="absolute top-4 right-4 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button onClick={() => handleEditStart(item)} aria-label="Edit item" className="p-1.5 text-slate-400 hover:text-emerald-600 bg-slate-50 rounded-md">
                       <Edit2 className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDelete(item.id)} aria-label="Delete item" className="p-1.5 text-slate-400 hover:text-rose-500 bg-slate-50 rounded-md">
                       <Trash2 className="w-4 h-4" />
