...
export const VetDashboard: React.FC<VetDashboardProps> = ({ userProfile }) => {
  const { pets, healthRecords, addRecord, appointments, updateAppointment, blockedSlots, addBlockedSlot, deleteBlockedSlot } = useStore();
  ...
  const stats = useMemo(() => {
    ...
    const avgConsultDuration = completedCount > 0 ? 45 : 0; // Simulated avg duration in minutes
    ...
    return [
        ...
        { label: 'Avg. Consult Duration', value: `${avgConsultDuration}m`, sub: 'Simulated Avg', icon: Clock, color: 'text-purple-500' },
