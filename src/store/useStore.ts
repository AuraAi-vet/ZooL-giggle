...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      syncStatus: (typeof window !== 'undefined' && typeof navigator !== 'undefined' && !navigator.onLine) ? 'offline' : 'synced',
      ...
      initializeCoreListeners: (userId: string) => {
        ...
        // Vets - only query role == 'vet' (fully allowed by secure rules)
        const vetsQuery = query(collection(db, 'users'), where('role', '==', 'vet'), limit(50));
        unsubscribes.push(onSnapshot(vetsQuery, (snapshot) => {
          const vets = snapshot.docs.map(doc => ({ uid: doc.id, ...doc.data() } as UserProfile));
          set({ vets });
        }, (error) => handleFirestoreError(error, OperationType.LIST, 'users (vets)')));

        // Admin - if authenticated as super admin, listen to all users
        if (auth.currentUser?.email === 'auraaivet@gmail.com') {
