...
import { calculatePetHealthScore } from './backend/services/healthScoringEngine';
...
async function startServer() {
  ...
  // Dashboard API Route
  app.get('/api/owner/pets/:petId/dashboard', async (req, res) => {
    try {
      const { petId } = req.params;
      
      const { data: petData, error } = await getSupabase()
        .from('pets')
        .select(`
          *,
          clinical_logs (*)
      ...
      const dynamicHealthScore = calculatePetHealthScore(petData as any, petData.clinical_logs as any);
      
      res.json({
        pet: petData,
        logs: petData.clinical_logs,
        computedScore: dynamicHealthScore
      });
    } catch (error) {
      console.error(error);
